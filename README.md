...
Все визуализации сделаны с помощью библиотеки matplotlib, ноутбук с построением этих графиков по ссылке: 
https://github.com/petrosbatu/sql_da_project/blob/main/visualization/visualization.ipynb
### 6.1.Товары
Наибольшую выручку приносят товары с большой ценой (кроме LG Dryer и LG Washing machine, т.е. бытовая техника). Большим спросом пользуются беспроводные наушники. Среди мониторов популярны те, которые имеют наибольшее разрешение/размер.

![prices_and_revenue](https://github.com/petrosbatu/sql_da_project/blob/main/visualization/plots/prices_and_revenue.png?raw=true)

Пять самых прибыльных товаров в каждом штате соответсвуют 6 самым прибыльным товарам в целом, а пятое место делится в зависимости от штата между 34in Ultrawide Monitor и Apple Airpods Headphones. 

![states](https://github.com/petrosbatu/sql_da_project/blob/main/visualization/plots/states.png?raw=true)

Тем не менее, дешёвые товары, а именно батарейки, кабели, недорогие наушники и др., часто встечаются в самых популярных парах товаров в заказах, а также впринципе часто покупаются в заказе вместе с чем-либо ещё. При этом популярны пары, в которых один товар один из самых дорогих, а другой - дешёвый.

![pairs](https://github.com/petrosbatu/sql_da_project/blob/main/visualization/plots/pairs.png?raw=true)
### 6.2.Покупатели

![Customers](https://github.com/petrosbatu/sql_da_project/blob/main/visualization/plots/customers.png?raw=true)

Возраст покупателей варьируется от 15 от 86 лет, средний равен 48, а медианный - 44. Покупателей можно разбить на две группы по прибыльности для компании, разделяя их по медиане возраста. Внутри каждой группы распределение равномерное, но при этом более молодые покупатели приносят больше прибыли компании.
...
Самых активных покупателей (совершивших 5 или более заказов) всего 336, при том что в БД хранятся данные о 140 тыс. клиентов. Следует учитывать, что компания проработала всего год, а постоянные клиенты, в среднем, делают 3-4 покупки в год.
### 6.3.Выручка
![revenue](https://github.com/petrosbatu/sql_da_project/blob/main/visualization/plots/time_revenue.png?raw=true)

Можно отметить месяцы с пиками прибыльности: апрель, октябрь и декабрь. Высокую выручку в декабре можно объяснить предстоящим Новым Годом. Апрель и октябрь требуют контекстаульной информации для более детального анализа.
